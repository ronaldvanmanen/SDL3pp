...
namespace boost
{
    namespace qvm
    {
        ...
        BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_OPERATIONS typename lazy_enable_if_c<
            is_vec<A>::value && vec_traits<A>::dim == 3 && is_quat<B>::value,
            deduce_vec2<A, B, 3>
        >::type
        operator*(A const & a, B const & b)
        {
            ...
            write_vec_element<0>(
                r,
                x * (qx * qx + qw * qw - qy * qy - qz * qz) + y * (2 * qx * qy - 2 * qw * qz) +
                    z * (2 * qx * qz + 2 * qw * qy)
            );
            write_vec_element<1>(
                r,
                x * (2 * qw * qz + 2 * qx * qy) + y * (qw * qw - qx * qx + qy * qy - qz * qz) +
                    z * (-2 * qw * qx + 2 * qy * qz)
            );
            write_vec_element<2>(
                r,
                x * (-2 * qw * qy + 2 * qx * qz) + y * (2 * qw * qx + 2 * qy * qz) +
...
sdl3::s_rgb96f shade(hit const & hit, world const & world, int level, float weight)
{
    ...
    for (auto const & light : world.lights)
    {
        ...
        if (amount_of_illumination > 0.0f && visibility > 0.0f)
        {
            ...
            color += hit.object.specular_coefficient * light.color *
                     powf(std::max(0.0f, dot(shading_normal, half_vector)), hit.object.specular_exponent);
        }
    }
    ...
    if (level < max_level)
    {
        ...
        if (reflective_weight > min_weight)
        {
            ...
            color += hit.object.reflective_coefficient * trace(reflected_ray, world, level + 1, reflective_weight);
        }
        ...
        {
            auto const eta = dot(shading_normal, hit.ray.direction) < 0.0f ? hit.object.index_of_refraction
                                                                           : 1.0f / hit.object.index_of_refraction;
            ...
            if (mag(transmitted_vector) != 0.0)
            {
                ...
                color += hit.object.transmissive_coefficient *
                         trace(transmitted_ray, world, level + 1, transmissive_weight);
            }
        }
    }
...
int main()
{
    ...
    auto texture = sdl3::streaming_texture<sdl3::pixel_format::rgb96f, sdl3::color_space::srgb>(
        renderer,
    ...
    world.lights.push_back(
        point_light(vector3{-300.0f, 350.0f, 10.0f}, sdl3::s_rgb96f(0.70_r32f, 0.689_g32f, 0.6885_b32f))
    );

    // Backdrop plane
    world.objects.insert(plane(
    ...
    // Default camera
    auto camera = perspective_camera{.field_of_view = 54.4f};

    camera.look_at_lh(vector3{0.0f, 8.5f, -32.0f}, vector3{0.0f, 8.25f, 0.0f}, vector3{0.0f, 1.0f, 0.0f});
            ...
            event.handle(
                sdl3::event_handler{
                    ...
                    [&window](sdl3::key_up_event && key_event) {
                        ...
                        if (symbol == sdl3::scan_code::m &&
                            modifiers == (sdl3::key_modifier::left_ctrl | sdl3::key_modifier::left_alt))
                        {
                            window.relative_mouse_mode(!window.relative_mouse_mode());
                        }
                    },
            ...
            for (auto raster_y = 0 * px; raster_y < raster_height; raster_y += 1 * px)
            {
                for (auto raster_x = 0 * px; raster_x < raster_width; raster_x += 1 * px)
                {
                    ...
                    auto const primary_ray = transform_ray(
                        ray(zero_vector, primary_ray_direction),
                    ...
                    auto const depth = (nearest_hit) ? std::clamp(
                                                           (nearest_hit->distance - world.min_depth) /
                                                               (world.max_depth - world.min_depth),
                                                           0.0f,
                                                           1.0f
                                                       )
                                                     : 1.0f;
