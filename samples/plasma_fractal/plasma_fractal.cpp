...
)
{
    ...
    actual_surface.with_lock(
        [&random_number_engine](sdl3::surface<sdl3::pixel_format::index8, sdl3::color_space::srgb> & image) {
            ...
            image(image_width - 1 * px, image_height - 1 * px) = sdl3::index8(
                random_number_distribution(random_number_engine)
            );
            ...
            for (auto step_size = image_height - 1 * px; step_size > 1 * px; step_size /= 2)
            {
                ...
                {
                    for (auto x = boost::units::fmod(y, step_size) == 0 * px ? half_step_size : 0 * px; x < image_width;
                         x += step_size)
                    {
