...
namespace sdl3::unit_test::tools
{
    ...
    public:
        virtual boost::unit_test::test_unit *
        next() const
        {
            if (!m_generated)
            {
...
#define TC_MAKE(z, arity, _)                                                       \
    ...
    {                                                                              \
        m_test_cases.push_back(new boost::unit_test::test_case(                    \
            genTestCaseName(),                                                     \
            m_tc_file,                                                             \
            m_tc_line,                                                             \
            boost::bind(                                                           \
                &TestCase::template test_method<BOOST_PP_ENUM_PARAMS(arity, Arg)>, \
    ...
    /**/
        ...
        operator()(Arg &&... arg) const
        {
            m_test_cases.push_back(new boost::unit_test::test_case(
                ...
                m_tc_line,
                std::bind(
                    &TestCase::template test_method<Arg...>,
    ...
    private:
        ...
        std::string
        generate_test_case_name(Head && head, Tail &&... tail) const
        {
        ...
        std::string
        generate_test_case_name(Head && head) const
        {
    ...
    {
        return boost::shared_ptr<boost::unit_test::test_unit_generator>(
...
#define SDL3PP_DATA_TEST_CASE_IMPL(arity, F, test_name, dataset, params)                          \
    struct BOOST_PP_CAT(test_name, case)                                                          \
    : public F                                                                                    \
    {                                                                                             \
        template <BOOST_PP_ENUM_PARAMS(arity, typename Arg)>                                      \
        static void                                                                               \
        test_method(BOOST_DATA_TEST_CASE_PARAMS(params))                                          \
        {                                                                                         \
            BOOST_TEST_CONTEXT("" BOOST_PP_SEQ_FOR_EACH(BOOST_DATA_TEST_CONTEXT, _, params))      \
            {                                                                                     \
    ...
    private:                                                                                      \
        ...
        void _impl(BOOST_DATA_TEST_CASE_PARAMS(params));                                          \
                                                                                                  ...
                                                                                                  \
    BOOST_AUTO_TEST_SUITE(test_name, *boost::unit_test::decorator::stack_decorator())             \
                                                                                                  \
        BOOST_AUTO_TU_REGISTRAR(BOOST_PP_CAT(test_name, case))(                                   \
            sdl3::unit_test::tools::make_data_test_case_generator<BOOST_PP_CAT(test_name, case)>( \
    ...
    BOOST_AUTO_TEST_SUITE_END()                                                                   \
                                                                                                  \
    template <BOOST_PP_ENUM_PARAMS(arity, typename Arg)>                                          \
    void BOOST_PP_CAT(test_name, case)::_impl(BOOST_DATA_TEST_CASE_PARAMS(params)) /**/

#define SDL3PP_DATA_TEST_CASE_WITH_PARAMS(F, test_name, dataset, ...) \
    SDL3PP_DATA_TEST_CASE_IMPL(                                       \
...
#define SDL3PP_DATA_TEST_CASE(...)                                                                                                                \
    BOOST_PP_CAT(                                                                                                                                 \
        BOOST_PP_IIF(BOOST_PP_EQUAL(BOOST_PP_VARIADIC_SIZE(__VA_ARGS__), 2), SDL3PP_DATA_TEST_CASE_NO_PARAMS, SDL3PP_DATA_TEST_CASE_WITH_PARAMS)( \
